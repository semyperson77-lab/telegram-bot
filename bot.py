...
def enter_hit(message):
    ...
    # If first in queue, start processing
    if len(uid_queue) == 1:
...
def process_next(message):
    ...
    # Always pick fresh UID data
    current = uid_queue[0]
    server = current["server"]
    uid = current["uid"]
    hits = current["hits"]        # ← FIX: EACH UID USES ITS OWN HITS
    ...
    # Prepare stats for all APIs
    total_stats = {}

    for api in apis:
        total_stats[api] = {"level": None, "likes": None, "success": 0}

    # HIT LOOP — EXACT hits only
